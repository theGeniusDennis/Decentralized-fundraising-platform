...
contract CrowdingFunding{
  ...
  //mapings to keep track of campaigns and [campaign ids to funders and contributions]
  ...
  //functionalites
  //remember to take off the duration
  function createCampaign(uint _goal, uint _dealine) external {
    ...
    emit campaignCreated(campaignCount, msg.sender, _goal, block.timestamp + _dealine);
  }
  ...
  function contribute(uint _campaignId) external payable {
    ...
    require(msg.value > 0, "Contributon should be created than zero");
  ...
  function withdrawFunds(uint _campaignId) external  {
    ...
    require(msg.sender == campaign.creator, "Only creator ca withdraw funds");
